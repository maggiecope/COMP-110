...
class Bear(Critter):
    """ A Bear in the critters world  """
    def __init__(self, location, grizzly):
        super().__init__(location)
        self.dirs = [Direction.NORTH, Direction.WEST]
        self.move_number = 0 
        #instance varible is_grizzly is set to grizzly 
        self.is_grizzly = grizzly
  
   #the bears string representation is "B"
    def __str__(self):
        return "B"
    
    #The bear moves in a north/west zigzag pattern, alternating between the two 
    def get_move(self, neighbors):
   ...
   #Eat is always True 
    ...
    #the color of the bear is "brown" when its a grizzly bear (is_grizzly==True), 
    # the color is "snow" when is_grizzly is not True and it is a polar bear
    def get_color(self): 
...
class Cheetah(Critter):
    """ A Cheetah in the Critters world """

    def __init__(self, location, hunger_count):
        super().__init__(location)
        self.dirs = [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST]
        self.move_number = 0 
        #instance variable hunger for hunger_count 
        self.hunger = hunger_count
        #instance variable, turns is set to zero, used in get_move to count the three moves in the same direction
        self.turn = 0 
        #instance variable that generates the first random choice of direction that will be later used in get_moves
        self.i = random.choice([Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST])
    
    #: Moves 3 steps in a random direction (north, south, east, or west),
    ...
    def get_move(self, neighbors):
        while self.turn<=3:
            if self.turn<3:
                ...
                #after three turns with the same random move
                #self turn is reset to zero 
                self.turn = 0 
                #a new random direction is chosen that will be used for the next three moves
                self.i = random.choice([Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST])
            self.move_number += 1
    
    #eat returns True the first "hunger" times it is called, False after that 
    def eat(self):
        #checking if hunger is greater than one 
        if self.hunger > 0:
            #subracts one from hunger 
            self.hunger -= 1
            #Eat is true 
            return True 
        else:
            #if there is no more hunger, returns False 
            return False
    
    #if the cheetah is hungry(if eat would return True), then scratch 
    # if the cheetah is not hungry(eat would return False), then pounce 
    def fight(self, opponent):
        #checking to see if the cheetah is hungry or not 
        if self.hunger>0:
            return Attack.SCRATCH
        else:
            return Attack.POUNCE

   # the color of the cheetah is "red"
    def get_color(self): 
        return "red"
    def __str__(self):
...
class Lion(Critter):
    ...
    def __init__(self, location):
        ...
        #instance variable self turn is set to zero 
        self.turn = 0
       
        #instance variable for hungry is orginally set to false, 
        # will be set to true if a fight occurs in def fight 
        self.hungry = False 

    #string representation for Lion is "L"
    def __str__(self):
        return "L"

    #the lion will first move south five times (self.dirs[0]), 
    # then west 5 times (self.dirs[1]), 
    # then north 5 times (self.dirs[2]), 
    # then east 5 times (self.dirs[3])
    #then it will repeat 
    def get_move(self, neighbors):
        while self.turn<=20:
            #for the first 5 turns returns south 
            if self.turn<5:
                self.turn+=1
                return self.dirs[0]
            #for the second 5 turns returns west 
            elif self.turn<10:
                self.turn+=1
                return self.dirs[1]
            #for the third 5 turns returns north 
            elif self.turn<15:
                self.turn+=1
                return self.dirs[2]
            #for the last five turns returns east 
            elif self.turn<20:
                self.turn+=1
                return self.dirs[3]
            #once a full 20 turns have been completed, 
            # self.turn is reset to 0 and the pattern will repeat again 
            else:
                self.turn = 0 

            self.move_number+=1
    #if opponent is a Bear ("B"), then roar 
    # if opponent is not bear then pounce 
    def fight(self, opponent):
        if opponent == "B":
            #if the lion gets into a fight/fights it will be hungry 
            self.hungry = True 
            #roars when opponent is bear 
            return Attack.ROAR
        else: 
            #if the lion gets into a fight/fights it will be hungry 
            self.hungry = True 
            #pounce when opponent is not bear 
            return Attack.POUNCE
    #eat returns True if the Lion has been in a fight since it last ate
    def eat(self):
        if self.hungry == True:
            #when the leion is hungry eat returns True 
            self.hungry = False
            #after it eats it will not be hungry again until it fights again 
            return True 
        else: 
            #if the lion has not fought since it last ate, it is not hungry and eat returns False 
            return False
    
    #The color of the Lion is "goldenrod3"
    def get_color(self): 
        return "goldenrod3"


class Torero(Critter):
        """ A Torero in the Critters World """
        def __init__(self, location):
            super().__init__(location)
            self.dirs = [Direction.SOUTH, Direction.WEST, Direction.NORTH, Direction.EAST]
            self.move_number = 0 
        ...
        #the string representation for the Torero is "B" - to trick my opponents 
        def __str__(self):
            return "B"
        #every other turn the torero will move randomly either north/south or west/east 
        def get_move(self, neighbors):
            while self.turn<=2:
                #first it will move randomly between north or south 
                if self.turn <1: 
                    return random.choice([Direction.NORTH, Direction.SOUTH])
                #next it will move randomly between west and east
                elif self.turn <2:
                    return random.choice([Direction.WEST, Direction.EAST])
                #self.turn is reset to 0 and this movement pattern will repeat 
                else: 
                    self.turn = 0 
                self.move_number += 1

        #the torero chooses its fight behavior based on its opponent
        def fight(self, opponent):
   
            if opponent == str(int):
                return random.choice([Attack.ROAR, Attack.SCRATCH])
            else:
                return Attack.POUNCE 


        #the torero will eat every three turns 
        def eat(self):
       ...
       #the color of the torero changes as the number of moves increases"
        def get_color(self): 
            if self.move_number<20:
                return "midnightblue"
            elif self.move_number<50:
                return "VioletRed1"
            elif self.move_number<100:
                return "red"
            elif self.move_number<150:
                return "magenta3"
            else: 
                return "magenta2"
